...
namespace questionmark.Application.Users.Commands
{
    ...
    {
        public UserLoginCommandValidator(IUser userRepository, IHashService hashService)
        {
            RuleFor(x => x.User).SetValidator(new UserLoginDtoValidator(userRepository, hashService));
        }
    }
    public class UserLoginCommand : IRequest<UserDetailsDto>
    {
        ...
        public class UserLoginCommandHandler : IRequestHandler<UserLoginCommand, UserDetailsDto>
        {
            ...
            public UserLoginCommandHandler(IJwtService jwtService, IHashService hashService, IEncryptionService encryptionService, IUser userRepository, ISession sessionRepository)
            {
            ...
            public async Task<UserDetailsDto> Handle(UserLoginCommand request, CancellationToken cancellationToken)
            {
                ...
                var user = await _userRepo.LoginUser(request.User, cancellationToken);
