...
namespace questionmark.Application.Posts.Commands
{
    ...
    {
        public CreateReactionCommandValidator(IPost postRepository)
        {
            RuleFor(x => x.Reaction).SetValidator(new CreateReactionDtoValidator(postRepository));
        }
    }
    public class CreateReactionCommand : IRequest<CreateReactionResponseDto>
    {
        public required CreateReactionDto Reaction { get; set; }
        public CreateReactionCommand(){}
        public class CreateReactionCommandHandler : IRequestHandler<CreateReactionCommand, CreateReactionResponseDto>
        {
            ...
            public CreateReactionCommandHandler(IJwtService jwtService, IHttpContextAccessor httpContextAccessor, Sessions.Contracts.ISession sessionRepo, IHashService hashService, IEncryptionService encryptionService, IPost postRepo)
            {
            ...
            public async Task<CreateReactionResponseDto> Handle(CreateReactionCommand request, CancellationToken cancellationToken)
            {
                ...
                var reaction = new CreateDomainReactionDto
                {
                    user_id = user_id,
                    currentReaction = request.Reaction.currentReaction,
                    postId = request.Reaction.postId,
                    date = DateTime.UtcNow
                };
                return await _postRepo.CreateReaction(reaction);
            }
        }
    }
}
